 ...
 * - Consumables focused (Food, Toys, Care, etc.)
 */
...
import {
    ...
    Bone, Heart, Scissors, Home, Package
} from 'lucide-react';
    ...
    const loadShopData = async () => {
        ...
        try {
            const catalogData = await getShopCatalog().catch(e => {
                console.error('[Supermarket] Catalog load failed:', e);
                return [];
            });

            // Filter to consumables only and add icons
            ...
            // Fallback mock data
            setItems([
                { id: 's1', sku: 'food-premium', name: 'Premium Kibble', description: 'Healthy daily food', price: 25, category: 'food', stock: 99, icon: '🍖' },
                { id: 's2', sku: 'toy-ball', name: 'Bouncy Ball', description: 'Fun for hours', price: 15, category: 'toy', stock: 99, icon: '🎾' },
                { id: 's3', sku: 'health-vitamins', name: 'Pet Vitamins', description: 'Boost health', price: 50, category: 'health', stock: 99, icon: '💊' },
            ]);
        } finally {
            setLoading(false);
        }
                        ...
                        <div style={{
                            ...
                            {CATEGORIES.map(cat => {
                                ...
                                const itemCount = cat.id === 'all'
                                    ? items.length
                                    : items.filter(i => i.category.toLowerCase() === cat.id || (cat.id === 'deals' && i.isDeal)).length;
...
export default SupermarketWindow;
