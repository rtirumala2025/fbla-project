...
export const TransactionTable: React.FC<{
  ...
  // Limit to recent transactions unless "Show All" is clicked
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Showing {displayed.length} of {sorted.length} transactions
            {!showAll && hasMore && ' (most recent)'}
          </p>
        </div>
        <div className="flex gap-3">
          {hasMore && (
            <button
              className="bg-gray-100 text-gray-700 px-4 py-2 rounded-pet text-sm font-semibold hover:bg-gray-200 transition-colors"
        ...
        <table className="min-w-full text-base">
          <thead>
            <tr className="text-left text-gray-600 border-b-2 border-gray-200">
              ...
              >
                Date {sortKey === 'created_at' && (sortDir === 'descending' ? '↓' : '↑')}
              </th>
              ...
              >
                Amount {sortKey === 'amount' && (sortDir === 'descending' ? '↓' : '↑')}
              </th>
            </tr>
          </thead>
          <tbody>
