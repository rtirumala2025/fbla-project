...
/**
 * AI insights dashboard that surfaces mood, personality, guidance,
 * and predictive health summaries for the pet.
 *
 * The component relies on server-side stubs that will eventually be replaced by
 * ML-driven services. The UI is structured to accommodate richer data without
 * requiring layout changes later (e.g., adding graphs or conversation history).
 */
export function PetAIDashboard() {
  ...
  const getAuthToken = useCallback(async () => {
    ...
    if (!token) {
      throw new Error('Authentication required to access AI insights.');
    }
    return token;
  }, []);
  ...
  const refreshInsights = useCallback(async () => {
    try {
      ...
      const [overview, alerts, helpResponse] = await Promise.all([
        getPetAIInsights({ token }),
        getPetAINotifications({ token }),
        getPetAIHelp({ token }),
      ]);
  ...
  const handleCommandSubmit = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        setError(null);
        const token = await getAuthToken();
        const result = await parsePetAICommand({ command_text: commandText }, { token });
