...
/**
 * Centralized care interface for the pet dashboard.
 *
 * The panel exposes key stats, AI reactions, and the recent diary timeline so
 * sprite/animation engineers can subscribe to stat changes and mood updates.
 * Future animation hooks can listen to the `reaction` string or `stats.mood`
 * to drive custom sprites without altering the data plumbing established here.
 */
export function PetCarePanel() {
  ...
  const updateFromAction = useCallback((response: PetActionResponse) => {
    if (response.pet?.stats) {
      setStats(response.pet.stats);
      setHealthSummary(computeHealthSummary(response.pet.stats));
    }
          ...
          <ul className="mt-4 space-y-4 overflow-auto pr-1 text-sm">
            ...
            {diary.map((entry) => (
              <li key={entry.id} className="rounded-md border border-slate-100 bg-slate-50 p-3">
                ...
                <p className="mt-2 text-slate-700">{entry.note || 'No note recorded.'}</p>
              </li>
            ))}
          </ul>
        </div>
