...
/**
 * Form allowing users to customize their pet.
 *
 * The sprite preview is intentionally simple for now; later tasks can swap in
 * animated components without changing the data flow established here.
 */
export function PetCustomizationForm({
  ...
  const [form, setForm] = useState<PetCreateRequest>(initialPet ? {
    ...
    color_pattern: initialPet.color_pattern,
    birthday: initialPet.birthday,
  } : defaultState);
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          value={form.color_pattern ?? ''}
            ...
            list="color-patterns"
          />
          {/* Suggested presets for easy selection */}
      ...
      {/* Lightweight preview; can be upgraded with real sprites later */}
