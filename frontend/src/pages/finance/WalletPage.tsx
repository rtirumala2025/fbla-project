...
export const WalletPage: React.FC = () => {
  ...
  const requireToken = useCallback(async () => {
    ...
    if (!token) {
      throw new Error('Authentication is required to access wallet data.');
    }
    return token;
  }, []);
  ...
  const handleClaimAllowance = async () => {
    ...
    try {
      setSubmitting(true);
      const token = await requireToken();
      const response = await claimDailyAllowance({ token });
      setSummary(response.summary);
  ...
  const handleCreateGoal = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await createGoal(
        { name: name.trim(), target_amount: parsedTarget },
        { token },
      );
      setSummary(response.summary);
  ...
  const handleContribute = async (goalId: string) => {
    ...
    try {
      setSubmitting(true);
      const token = await requireToken();
      const response = await contributeGoal(goalId, { amount }, { token });
      setSummary(response.summary);
