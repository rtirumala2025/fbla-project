...
export const Login = () => {
  ...
  const { signIn, signInWithGoogle, currentUser, signOut } = useAuth();

  // Handling for user who is already logged in
  if (currentUser) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-cream py-16 px-4 sm:px-6 lg:px-8">
        <motion.div
          key="logged-in-view"
          className="max-w-[500px] w-full space-y-8 p-10 bg-white rounded-pet shadow-soft text-center"
