...
An end-to-end virtual pet simulator that teaches financial literacy through gameplay. Students adopt a companion, manage virtual budgets, play adaptive mini-games, and receive AI-driven coaching—all powered by a React frontend, FastAPI backend, and Supabase storage.
...
- **3D Environment** – Optimized Dog Park with day/night cycles, dynamic camera controls, and improved performance for laptops.
- **Demo Mode** – One-click "Reset to Demo State" in Settings to instantly prepare the app for competition presentation.
- **Accessibility & Inclusivity** – Theme toggle, reduced motion, high contrast, and offline caching built into the UI.
- **Competition Utilities** – Seed/demo data script, coverage reports, presentation deck, and user manual prepared for judges.

## System Architecture
- **Frontend (`frontend/`)** – React + TypeScript SPA, Framer Motion transitions, React Context for auth/theme/pet state, and Supabase client for realtime updates.
- **Backend (`app/`)** – FastAPI service exposing domain routers for auth, pets, finance, analytics, games, and next-gen exploratory features.
- **Database (Supabase Postgres)** – Auth + row-level security with `profiles`, `user_preferences`, `pets`, and additional finance/shop tables inserted via seeding script.
- **AI & Analytics Services** – Service layer abstracts mood scoring, financial summaries, leaderboard logic, and next-gen projections.
- **Observability & Testing** – Pytest suites for backend services, Playwright/E2E coverage for critical flows, and coverage report.
...
- `docs/architecture-diagram.{mmd,svg}` – High-level system architecture (frontend ↔ backend ↔ Supabase).
- `docs/supabase-erd.{mmd,svg}` – Relational model covering auth, pets, finance, and inventory tables.
- `docs/user-manual.{md,pdf}` – End-user guide; PDF generated via `scripts/generate_assets.py`.
- `docs/presentation-deck.{md,pdf}` – Slide outline and ready-to-present PDF deck.
- `docs/demo-storyboard.{md,pdf}` – Scene-by-scene plan aligning with the demo video.
- `docs/demo-script.md` – Step-by-step live demo script with timing cues.
- `docs/demo-video.mp4` – Downloadable fallback walkthrough (replace with narrated recording).
- `scripts/seed_demo_data.sql` – Comprehensive Supabase seeding for wallets, inventory, and analytics.
- `scripts/seed_competition_data.sql` – Lightweight seed for rapid resets between judge demos.
- `reports/test-coverage.md` – Latest coverage snapshot and interpretation.
- `LICENSE` – MIT license for competition submission.
...
Good luck in competition—refer to the user manual for talking points, and pair the presentation deck with the demo script for a polished run-through. You've got this! 💪
